...
function normalizeBitacoraProject() {
  const project = state.projects.find((item) => item.title === "Bitacora Cyber personal");
  if (!project) return;

  const shouldUpdate = project.status === "En progreso" || !project.summary || !project.objective;
        ...
        .map((project) => `
          <article class="project-card">
            <div>
              ...
              <p>${escapeHtml(project.summary || project.tech || "Proyecto tecnico documentado.")}</p>
              <div class="project-meta">
                <span>${escapeHtml(project.tech || "Tecnologias por definir")}</span>
              </div>
              <small>${formatDate(project.start)}${project.end ? ` - ${formatDate(project.end)}` : " - En curso"}</small>
              <div class="project-detail">
                ${entrySection("Objetivo", project.objective)}
                ${entrySection("Resultados", project.results)}
                ${entrySection("Lecciones", project.lessons)}
              </div>
              <div class="form-actions">
                ${project.link ? `<a class="ghost-button" href="${escapeHtml(project.link)}" target="_blank" rel="noreferrer">Ver proyecto</a>` : ""}
                <button class="ghost-button project-report-button" data-id="${project.id}" type="button">Ver ficha</button>
                <button class="primary-button project-download-button" data-id="${project.id}" type="button">Descargar ficha</button>
              </div>
            </div>
...
function getFormData(formType) {
  ...
  if (formType === "project") {
    return {
      ...
      objective: document.querySelector("#project-objective").value.trim(),
      procedure: document.querySelector("#project-procedure").value.trim(),
      results: document.querySelector("#project-results").value.trim(),
      lessons: document.querySelector("#project-lessons").value.trim(),
    };
  }
...
function editItem(type, id) {
  ...
  if (type === "project") {
    ...
    document.querySelector("#project-link").value = project.link || "";
    document.querySelector("#project-summary").value = project.summary || "";
    document.querySelector("#project-objective").value = project.objective || "";
    document.querySelector("#project-procedure").value = project.procedure || "";
    document.querySelector("#project-results").value = project.results || "";
    document.querySelector("#project-lessons").value = project.lessons || "";
...
function bindEvents() {
  ...
  document.querySelectorAll(".export-data-button").forEach((button) => {
    button.addEventListener("click", exportPublishedData);
  });
}
...
function bindSharedEvents() {
  document.addEventListener("click", (event) => {
    ...
    const reportButton = event.target.closest(".project-report-button");
    const downloadButton = event.target.closest(".project-download-button");
    ...
    if (reportButton) openProjectReport(reportButton.dataset.id);
    if (downloadButton) downloadProjectReport(downloadButton.dataset.id);
  });
...
function projectReportHtml(project) {
  const field = (label, value) => value ? `<section><h2>${label}</h2><p>${escapeHtml(value).replaceAll("\n", "<br>")}</p></section>` : "";
...
<head>
  ...
  <title>Ficha tecnica - ${escapeHtml(project.title)}</title>
  <style>
    body { font-family: Arial, sans-serif; margin: 40px; color: #14202a; line-height: 1.55; }
    header { border-bottom: 3px solid #0f766e; margin-bottom: 24px; padding-bottom: 16px; }
    h1 { margin: 0 0 8px; font-size: 30px; }
    h2 { color: #0f766e; margin: 24px 0 8px; font-size: 18px; }
    .meta { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 8px 18px; color: #334155; }
    .pill { display: inline-block; padding: 4px 9px; border-radius: 999px; background: #ccfbf1; color: #115e59; font-weight: 700; }
...
<body>
  <header>
    ...
    <div class="meta">
      <div><strong>Inicio:</strong> ${formatDate(project.start)}</div>
      <div><strong>Fin:</strong> ${project.end ? formatDate(project.end) : "En curso"}</div>
      <div><strong>Tecnologias:</strong> ${escapeHtml(project.tech || "No especificadas")}</div>
      <div><strong>Enlace:</strong> ${project.link ? `<a href="${escapeHtml(project.link)}">${escapeHtml(project.link)}</a>` : "No publicado"}</div>
    </div>
  </header>
  ${field("Resumen", project.summary)}
  ${field("Objetivo", project.objective)}
  ${field("Procedimiento tecnico", project.procedure)}
  ${field("Resultados", project.results)}
  ${field("Lecciones aprendidas", project.lessons)}
</body>
</html>`;
}
...
function openProjectReport(id) {
  ...
  reportWindow.document.write(projectReportHtml(project));
  reportWindow.document.close();
}

function downloadProjectReport(id) {
  const project = getProjectById(id);
  if (!project) return;

  const slug = project.title.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/(^-|-$)/g, "") || "proyecto";
  const blob = new Blob([projectReportHtml(project)], { type: "text/html" });
  const url = URL.createObjectURL(blob);
  const link = document.createElement("a");
  link.href = url;
  link.download = `${slug}-ficha-tecnica.html`;
  link.click();
  URL.revokeObjectURL(url);
}
